...
def main(config_file):
    ...
    if config['lr_scheduler']['type'].lower() == 'onecyclelr':
        ...
        config['lr_scheduler']['steps_per_epoch'] = len(train_loader)
    ...
    if discriminator:
        if config['lr_scheduler_d']['type'].lower() == 'onecyclelr':
            ...
            config['lr_scheduler_d']['steps_per_epoch'] = len(train_loader)
    ...
    m_loss_log = []
    d_loss_log = []
    m_loss_fname = os.path.join(results_folder, 'model_log_loss.dat')
    d_loss_fname = os.path.join(results_folder, 'disc_log_loss.dat')
    for epoch in range(num_epochs):
        ...
        for bstep, X in enumerate(progress_bar):
            ...
            np.savetxt(m_loss_fname, t, header =','.join(list(m_msg.keys())) )
            t = np.array(d_loss_log)
            np.savetxt(d_loss_fname, t, header = ','.join(list(d_msg.keys())) )
